...
class App:
    ...
    *** USE THIS EXACT DATE - DO NOT CALCULATE OR GUESS ***
    ...
    def _route_command(self, raw_text: str) -> bool:
        ...
        # Personality voice commands
        # Add to your existing personality_commands:
        # Add to your existing personality_commands:
            ...
            if command in text:
                if personality in self.personalities:
                    ...
                    if current_personality == personality:
                        self.speak_search_status(f"Already in {personality} mode")
                    else:
                        self.personality_var.set(personality)
                        self.apply_personality(personality)
                        if personality == "Default":
                            self.speak_search_status("Returning to normal mode")
                        else:
    ...
    def filter_whisper_hallucinations(self, text: str) -> str:
        ...
        hallucinations = [
            ...
            "see you next time", "until next time", "bye everyone",
            "welcome back", "in this video", "before we start"
        ]

        text_lower = text.lower().strip()
        self.logln(f"[asr-filter] Checking: '{text}'")
        ...
        for hallucination in hallucinations:
            if text_clean == hallucination:
                self.logln(f"[asr-filter] ❌ EXACT MATCH FILTERED: '{text}'")
                return ""

        # Also check if it starts with common hallucination phrases
        for hallucination in hallucinations:
            if text_lower.startswith(hallucination):
                self.logln(f"[asr-filter] ❌ STARTS WITH FILTERED: '{text}'")
                return ""

        self.logln(f"[asr-filter] ✅ PASSED: '{text}'")
        return text
